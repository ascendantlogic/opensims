...
// SET SCREEN
_global.SCREEN = _root.UI_CONTENT;
...
var XY_Positions = function(ROLE:String, THREAT:String, TARGET:MovieClip) {
  ...
  // hardcoded for widget
	 ...
	 TARGET.ATKDEF_ROLE_BOTTOM = 200;
	 TARGET.ATKDEF_ROLE_LEFT = 110;
	 TARGET.ATKDEF_ROLE_RIGHT = _global.ATKDEF_WIDTH-TARGET.ATKDEF_ROLE_LEFT-15;
...
var drawATKDEFLine = function(TARGET, src, dst, ANGLE, TARGET_COLOR, IDS, dst_id) {
  
  /*
  if (_global.DEBUG) {
	 _global.log("drawATKDEFLine src:"+src+" dst:"+dst+" CON_COLOR:"+CON_COLOR);
  }
  */
...
SCREEN.drawATKDEFBorders = function(TARGET:MovieClip) {
  ...
  // straight lines on two Xs
	 ...
	 // ATK
	 border_mc.lineStyle(1, 0x660000, _global.ALPHA);
    ...
    // line not already there, src and dst are real
	 if (!TARGET[LINEKEY] && (src_id != dst_id) && (src.x && dst.x)) {
      ...
      // CALCULATE ANGLE
		...
		drawATKDEFLine(LINEKEY_mc, src, dst, ANGLE, THIS_COLOR, this.DATA.CON[i]['ids'], dst_id);
...
SCREEN.drawATKDEFPanes = function() {
  ...
  var PANE = SCREEN.createEmptyMovieClip("ATKDEF_"+_global.VIZ_THREAT.toUpperCase(), _global.getNewDepth());
  PANE.CONTENT = PANE.createEmptyMovieClip("CONTENT", _global.getNewDepth());
